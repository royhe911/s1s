...
class Excel
{
     ...
     * @param array $res_data 待组装数据
     * @param array $branch_no 版本号
     * @param mixed $insertFunc 写入数据库的回调函数，可以用匿名函数
     * @param string $type Excel2007|Excel5
     * @return int
     */
    public static function parse($file, $header, $perLimit, $res_data, $branch_no, $insertFunc, $type = '')
    {
                    ...
                    foreach ($cellIterator as $cell) {
                        if ($objPHPExcel->getActiveSheet()->getColumnDimension($cell->getColumn())->getVisible()) {
                            if (isset($header[$cell->getColumn()])) {
                                $rowData[$header[$cell->getColumn()]] = $cell->getCalculatedValue();
                            }
                        }
                    }
                    $rowData['create_time'] = date("Y-m-d H:i:s");
                    $rowData['branch_no'] = $branch_no ?? '';
                    $rowData['images'] = $rowData['detail_images'] = [];
                    if (!empty($res_data)) {
                        $file_keys = $res_data['file_keys'];
                        $rowData['images'] = isset($file_keys[trim($rowData['images_arr'], '/')]) ? $file_keys[trim($rowData['images_arr'], '/')] : [];
                        if (isset($rowData['detail_image'])) {
                            $rowData['detail_image'] = isset($file_keys[trim($rowData['detail_image'], '/')]) ? $file_keys[trim($rowData['detail_image'], '/')] : [];
                        }
                        $rowData['shop_id'] = isset($res_data['shop_id']) ? $res_data['shop_id'] : '';
                        $rowData['city_id'] = isset($res_data['region_id_2']) ? $res_data['region_id_2'] : '';
                        $rowData['member_id'] = isset($res_data['member_id']) ? $res_data['member_id'] : '';
                    }
                    $data[] = $rowData;
                    $count++;
    ...
    /**
     * 解析 Excel
     * @param $file Excel 路径名文件名
     * @param $header 表头对应字段信息 ['A'=>'field1', 'B'=>'field2', ...]
     * @param $type Excel2007|Excel5
     * @return array
     */
    public static function parseArray($file, $header, $type)
    {
        $type = self::getType($file, $type);
        $objReader = PHPExcel_IOFactory::createReader($type);
        $objPHPExcel = $objReader->load($file);
