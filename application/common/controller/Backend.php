...
class Backend extends Controller
{
    ...
    public function initialize()
    {
        ...
        $pathArr = explode('.', $controllername);
        $pathArr[] = $actionname;
        foreach ($pathArr as &$value) $value = ltrim(strtr($value, $str_replace), '_');
        ...
        if (!$this->auth->match($this->noNeedLogin)) {
            ...
            // 判断是否需要验证权限
            $AuthRuleAll = array_column(AuthRule::getDataAll(), 'name');

            if (array_search($path, $AuthRuleAll) !== false) {
                if (!$this->auth->match($this->noNeedRight)) {
                    // 判断控制器和方法判断是否有对应权限
                    if (!$this->auth->check($path)) {
                        Hook::listen('admin_nopermission', $this);
                        //if ()
                        //return $this->result(null, 0, '您没有操作的权限', 'html');
        ...
        $upload = \app\admin\model\Config::upload();
        ...
        $config = [
            ...
            'language' => $lang,
            'fastadmin' => Config::pull('fastadmin'),
        ...
        // 页码
        if (request()->get('cur_limit')) {
            \session('cur_limit', request()->get('cur_limit'));
        }
        $this->cur_limit = \session('cur_limit') ?: $this->default_limit;
    ...
    {
        Lang::load(APP_PATH . Request::module() . '/lang/' . Lang::detect() . '/' . str_replace('.', '/', $name) . '.php');
    }
     ...
     */
    protected function getRegionsByAuth()
    {
        $adminInfo = Session::get('admin');

        $auth_group_id = AuthGroupAccess::getByUid($adminInfo['id']);
        $city_group = AuthGroup::where('pid|id', 2)->column('id');

        $w[] = ['level_type', '=', 2];
        if (in_array($auth_group_id, $city_group)) {
            $support_city = ConfigM::getValue('support_city');
            $w[] = ['id', 'in', $support_city];
        }
        return Regions::where($w)->group("id")->select()->toArray();
    }
}
