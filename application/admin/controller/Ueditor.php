...
class Ueditor
{
    ...
    public function config()
    {
        $config = [
            ...
            "imageAllowFiles"         => [".png", ".jpg", ".jpeg", '.bmp'], /* 上传图片格式显示 */
    ...
    {
        $prefix2 = Request::param('prefix');
        $pre = Config::get('qiniu.img_prefix');
        // 1 - 商品  2 - 资讯  3 -
        $zh = 'test';
        if ($prefix2 == 1) {
            $zh = "good/ueditor/";
        ...
        $prefix = $pre . $zh . date("YmdHis") . mt_rand(10000, 999999) . '/';
...
//        }
        if (!empty($error)) {
            ...
            if ($info[0]['type'] != 'video/mp4') {
                $info[0]['key'] = $info[0]['key'];
            } else {
                $info[0]['key'] = config('qiniu.domain') . $info[0]['key'];
            }
            $ret = [
                "state"    => 'SUCCESS',
                "url"      => $info[0]['key'],
    ...
    public function listImage()
    {
        ...
        $model = Db::connect("db_admin")->name("File");
