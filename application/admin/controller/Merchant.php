...
class Merchant extends Backend
{
    ...
    public function index()
    {
        ...
        if ($keyword != null && !empty($keyword)) {
            $w[] = ['p.name|p.company_name', 'like', '%' . $keyword . '%'];
        }
        if ($status >= 0) {
            $w[] = ['a.status', '=', $status];
        }
    ...
    public function add()
    {
        if (request()->isAjax()) {
            ...
            if (empty($city_arr) || !is_array($city_arr)) {
                ...
                if (count(array_unique($parent_id_arr)) != 1) {
                    $this->error(__('不可选择跨省的城市！'));
                }
            ...
            try {
                ...
                foreach ($city_arr as $v) {
                    $data[] = ['region_id' => $v, 'admin_id' => $admin_id];
                }
            ...
            if (isset($admin_id)) {
                ...
                $mes = '尊敬的商家: 请在收到激活邮件后的48小时内，通过点击下面的链接进行操作，并且设置新的登录密码进行激活
                ...
                \Mail::sendEmail($email, '刷一刷后台激活通知', $mes);
                // 发送短信
                sendSms(3, $mobile);
