define(['jquery', 'bootstrap', 'moment', 'moment/locale/zh-cn', 'bootstrap-table', 'bootstrap-table-lang', 'bootstrap-table-export', 'bootstrap-table-commonsearch', 'bootstrap-table-template'], function ($, undefined, Moment) {
    var Table = {
        ...
        config: {
            ...
            editbtn: '.btn-edit',
            delbtn: '.btn-del',
            status_enable_btn: '.btn-status-enable',
            status_disable_btn: '.btn-status-disable',
        ...
        api: {
            ...
            bindevent: function (table) {
                ...
                // 批量启用
                $(toolbar).on('click', Table.config.status_enable_btn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
                    Layer.confirm(
                        __('Are you sure you want to enable the %s selected item?', ids.length),
                        {icon: 3, title: __('Warning'), offset: 0, shadeClose: true},
                        function (index) {
                            Table.api.multi("status_enable", ids, table, that);
                            Layer.close(index);
                        }
                    );
                });
                // 批量禁用
                $(toolbar).on('click', Table.config.status_disable_btn, function () {
                    var that = this;
                    var ids = Table.api.selectedids(table);
                    Layer.confirm(
                        __('Are you sure you want to disable the %s selected item?', ids.length),
                        {icon: 3, title: __('Warning'), offset: 0, shadeClose: true},
                        function (index) {
                            Table.api.multi("status_disable", ids, table, that);
                            Layer.close(index);
                        }
                    );
                });
            ...
            multi: function (action, ids, table, element) {
                ...
                var action2 = '';
                if (action == 'del' || action == 'status_enable' || action == 'status_disable') {
                    var aa_url = action + '_url';
                    var aa2 = options.extend;
                    action2 = aa2[aa_url] ? aa2[aa_url] : '';
                }
                //var action2 = (action == "del" ? options.extend.del_url : options.extend.multi_url);
                var url = typeof data.url !== "undefined" ? data.url : action2;
